...
Pasager::Pasager(): nr_zbor("000000")
{
	/*this->nume = " ";
	this->prenume = " ";*/
...
Pasager::Pasager(char* nume, char* prenume, long long CNP, int nrBagaje, string seria, string numar, string nr_zbor): nr_zbor(nr_zbor)
{
...
Pasager::Pasager(const Pasager& p):nr_zbor(p.nr_zbor)
{
	char buffer1[200], buffer2[200];
...
void Pasager::setNrZbor(string nr_zbor)
{
	if ( nr_zbor != this->nr_zbor)
		this->nr_zbor = nr_zbor;
	else throw "ERROR: NUMAR DE ZBOR INVALID!";
}
...
ostream& operator<<(ostream& output, const Pasager& p)
{
	...
	output << "NUMAR ZBOR: " << t << t << t << p.nr_zbor << n;

	return output;
	// // O: insert return statement here
}

ofstream& operator<<(ofstream& outputfile, const Pasager& p)
{
	//outputfile << "NUME  PRENUME  CNP  NUMAR BAGAJE  SERIE  NUMAR DOCUMENT  NUMAR ZBOR" << n;
	outputfile << p.nume << " " << p.prenume << " " << p.CNP << " " << p.nrBagaje << " " << p.seria << " " << p.numar << " " << p.nr_zbor;
	return outputfile;
	// // O: insert return statement here
}

ifstream& operator>>(ifstream& inputfile, Pasager& p)
{
	char buffer1[100];
	inputfile >> buffer1;
	p.nume = new char[strlen(buffer1) + 1];
	...
	inputfile >> p.CNP >> p.nrBagaje >> p.seria >> p.numar >> p.nr_zbor;
